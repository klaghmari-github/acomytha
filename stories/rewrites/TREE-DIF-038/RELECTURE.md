# TREE-DIF-038 — Le cheval de bois de Nino, sous l'auvent

- **Public :** N1 (3–4 ans), audio familial
- **Leçon :** DIF.PAR.002 — laisser l'autre finir sa phrase (vécue)
- **Personnages :** Nino, papa, maman
- **Lieu :** sous l'auvent, après la pluie : établi, coffre, étagère
- **Structure conservée :** 86 nœuds, 27 chemins, 27 fins textuellement distinctes

Relu : monde, désir, imprévu, question, résolution, fin heureuse. `chunk_id` / `kind` / graphe `option_*_next` inchangés.

## Promesse narrative

Sous l'auvent, les carreaux sèchent. Nino veut que son cheval de bois **galope maintenant**. Une roue manque. Papa commence : « La roue est dans le. » Nino coupe : « Dans le quoi ? » Le mot rentre. Première idée ratée. Il prend le cheval, la boîte ronde ou le chiffon ; l'établi cache, le coffre pèse ou l'étagère est trop haute ; une action change l'écoute (petit tiroir, copeaux, tiroir bas ; coin gauche, deux mains, cale ; pot bleu, tabouret, bras). La phrase arrive. La roue rentre. Le cheval galope.

## Vécu

Nino veut le galop **maintenant**. Il parle à la place de papa. Silence, mot perdu, cheval sur trois roues. Chaque choix change l'obstacle et le climax. La leçon se voit : couper donne une bouche fermée ; laisser finir donne le petit tiroir, l'étiquette, le bas, le coin, les deux mains, la cale, le pot bleu, le tabouret, les bras. Fin : quatre roues + carreaux secs, image unique du chemin (copeau, toc, rai, savon).

## Vu et corrigé

- Slogan salon / Noé / bac-toboggan-balançoires / Tom-Léa-Sami jetés.
- Tics « encore / déjà / tout doux / tout calme » retirés.
- T1/T2/T3 changent l'action, pas seulement le décor. 9 T2 distincts, 27 T3, 27 fins.
- Merci vécu (cheval tenu). Question d'adulte. Un « en ce moment ».
- TTS complet (86) : `text_ssml`, `text_xai_tags`, `notes` (arc, intention, émotion, intensité, destinataire, sous-texte, tempo, sourire, respiration). `slow` = choix, indice, fin. Action plus vive.
- N1 ≤ 10. `check()` OK. Pas apply.

## Contrôles

- 86 chunks, 27 chemins, 27 fins distinctes, 27 dernières images
- 416 à 436 mots par chemin (moyenne 426)
- `text` = `script` collé ; graphe inchangé

## Non vérifié

Audio (pas cuit). Durée réelle à l'écoute. Playtest moteur des 27 chemins.
