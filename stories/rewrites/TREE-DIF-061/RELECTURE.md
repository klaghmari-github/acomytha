# TREE-DIF-061 — Le moulin de papier d'Aniss et la grille de l'école

- **Public :** N3 (5–6 ans), audio familial
- **Leçon :** DIF.PAR.001 — camarade qui parle peu / tendre, attendre (vécue, jamais dite)
- **Personnages :** Aniss, Mila, papa, maman
- **Lieu :** chemin de l'école : grille, caniveau, porche
- **Structure conservée :** 86 nœuds, 27 chemins, 27 fins textuellement distinctes

Relu : monde, désir, imprévu, question, résolution, fin heureuse. `chunk_id` / `kind` / graphe `option_*_next` inchangés.

## Promesse narrative

Un bus passe ; le papier répond depuis le sac. Aniss sort le moulin : une goutte de cire rouge tient une pale. Il veut le planter à la grille avant que le vent ne se couche sous le porche. Mila veut qu'il crie tourne. Aniss lève trop vite : le bâton bute. Sourire parti. Moulin, fil, caillou : les trois partent. Grille (barreaux trop serrés), caniveau (eau trop vite), porche (air trop sage). Bas, mains de Mila, crochet ; dalle, fil, bord ; vent, marche, nœud. La goutte de cire rouge prend le vent, avec une trace.

## Vécu

Aniss veut le moulin **à la grille, maintenant**. Mila ne veut pas la même chose : elle veut des mots. Première idée : lever trop vite. Ça rate. Chaque choix change l'obstacle et le climax (fer, eau, ombre). La leçon se voit : Aniss tend, Mila attend, le silence compte. On ne force pas la parole, on change le geste. Fin : goutte de cire rouge + image unique du chemin.

## Vu et corrigé

- Zoé / Tom / Léa / Sami / Lila / cuisine-jardin-chambre / « on va apprendre » jetés.
- Tics « encore / déjà / tout doux / tout calme / tout lent » retirés.
- Héros Aniss (`enfant-m`), Mila (`copine`), rythmes distincts, silence = réponse.
- T1 ne retire pas l'équipement. T1/T2/T3 changent l'action. 9 T2, 27 T3, 27 fins.
- Indice unique : goutte de cire rouge (ouverture + climax). Pas d'ancre / étoile / fil pâle / marque fine / pastille de colle.
- Ouverture inventée : le bus, le papier qui répond dans le sac.
- Corps : sourire parti ; envie et inquiétude ; papa s'accroupit.
- Merci vécu (ouverture). Question d'adulte. Un « en ce moment ».
- Monde ≠ TREE-DIF-034 (pas de soleil en papier, vestiaire, pastille).
- Adultes = papa/maman. Pas de maîtresse.
- TTS complet (86) : `text_ssml`, `text_xai_tags`, `notes` (arc, intention, émotion, intensité, destinataire, sous-texte, tempo, sourire, respiration). `slow` = choix, indice, fin. Action plus vive.
- N3 ≤ 16. `check()` OK. Pas apply.

## Contrôles

- 86 chunks, 27 chemins, 27 fins distinctes, 27 dernières images
- 629 à 644 mots par chemin (moyenne 636)
- `text` = `script` collé ; graphe inchangé

## Non vérifié

Audio (pas cuit). Durée réelle à l'écoute. Playtest moteur des 27 chemins.
